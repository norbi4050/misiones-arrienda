-- =====================================================================================
-- ESQUEMA SQL SUPABASE - PARTE 2: POLÍTICAS RLS Y CONFIGURACIÓN DE STORAGE
-- =====================================================================================
-- Continuación del esquema principal para Misiones Arrienda
-- =====================================================================================
...
CREATE POLICY "Profile owners can manage their rooms" ON rooms FOR ALL USING (
    EXISTS (SELECT 1 FROM user_profiles WHERE user_profiles.id = rooms.owner_id AND user_profiles.user_id = auth.uid()::text)
);

-- Políticas para likes
CREATE POLICY "Users can view likes involving their profile" ON likes FOR SELECT USING (
    EXISTS (SELECT 1 FROM user_profiles WHERE user_profiles.id = likes.from_id AND user_profiles.user_id = auth.uid()::text)
    OR
    EXISTS (SELECT 1 FROM user_profiles WHERE user_profiles.id = likes.to_id AND user_profiles.user_id = auth.uid()::text)
);
...
CREATE POLICY "Users can view their own conversations" ON conversations FOR SELECT USING (
    EXISTS (SELECT 1 FROM user_profiles WHERE user_profiles.id = conversations.a_id AND user_profiles.user_id = auth.uid()::text)
    OR
    EXISTS (SELECT 1 FROM user_profiles WHERE user_profiles.id = conversations.b_id AND user_profiles.user_id = auth.uid()::text)
);
...
CREATE POLICY "Users can view messages in their conversations" ON messages FOR SELECT USING (
    EXISTS (
        ...
        AND (up_a.user_id = auth.uid()::text OR up_b.user_id = auth.uid()::text)
    )
);
CREATE POLICY "Users can send messages from their profile" ON messages FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM user_profiles WHERE user_profiles.id = messages.sender_id AND user_profiles.user_id = auth.uid()::text)
);
...
-- Políticas para storage de imágenes de propiedades
...
CREATE POLICY "Users can update their property images" ON storage.objects FOR UPDATE USING (
    bucket_id = 'property-images' AND auth.uid()::text = owner
);
CREATE POLICY "Users can delete their property images" ON storage.objects FOR DELETE USING (
    bucket_id = 'property-images' AND auth.uid()::text = owner
);

-- Políticas para storage de imágenes de perfil
...
CREATE POLICY "Users can update their profile images" ON storage.objects FOR UPDATE USING (
    bucket_id = 'profile-images' AND auth.uid()::text = owner
);
CREATE POLICY "Users can delete their profile images" ON storage.objects FOR DELETE USING (
    bucket_id = 'profile-images' AND auth.uid()::text = owner
);

-- Políticas para storage de imágenes de comunidad
...
CREATE POLICY "Users can update their community images" ON storage.objects FOR UPDATE USING (
    bucket_id = 'community-images' AND auth.uid()::text = owner
);
...
BEGIN
    SELECT json_build_object(
        ...
        'average_price', (SELECT AVG(price) FROM properties WHERE status = 'AVAILABLE'),
        'price_ranges', (
            SELECT json_build_object(
...
BEGIN
    ...
    SELECT 
        ...
        (
            ...
            CASE WHEN ABS(p.price - bp.price) / bp.price < 0.2 THEN 3 ELSE 0 END +
...
SELECT 
    ...
    up_a.photos[1] as user_a_photo,
    up_b.photos[1] as user_b_photo
...
CREATE INDEX idx_properties_title_gin ON properties USING gin(to_tsvector('spanish', title));
CREATE INDEX idx_properties_description_gin ON properties USING gin(to_tsvector('spanish', description));
CREATE INDEX idx_properties_address_gin ON properties USING gin(to_tsvector('spanish', address));
...
CREATE INDEX idx_user_profiles_tags_gin ON user_profiles USING gin(tags);
CREATE INDEX idx_rooms_amenities_gin ON rooms USING gin(amenities);

-- Índices para mensajería
CREATE INDEX idx_messages_unread ON messages(conversation_id, is_read, created_at) WHERE is_read = false;
...
-- =====================================================================================
-- FUNCIÓN DE VERIFICACIÓN DEL SETUP
-- =====================================================================================
