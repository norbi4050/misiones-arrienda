-- ============================================
-- SISTEMA DE AUTO-EXPIRACIÓN DE PLANES (FINAL)
-- ============================================
-- FIX: Compatibilidad entre TEXT (users.id) y UUID (properties.user_id)
-- ============================================

-- ============================================
-- 1. FUNCIÓN: Expirar plan y downgrade a FREE
-- ============================================
DROP FUNCTION IF EXISTS expire_user_plan(TEXT);
DROP FUNCTION IF EXISTS expire_user_plan(UUID);

CREATE OR REPLACE FUNCTION expire_user_plan(user_uuid TEXT)
RETURNS TABLE (
  success BOOLEAN,
  old_plan VARCHAR,
  new_plan VARCHAR,
...
BEGIN
  ...
  -- FIX: Cast explícito user_uuid::uuid
...
-- ============================================
DROP FUNCTION IF EXISTS deactivate_excess_properties(TEXT, INTEGER);
DROP FUNCTION IF EXISTS deactivate_excess_properties(UUID, INTEGER);

CREATE OR REPLACE FUNCTION deactivate_excess_properties(user_uuid TEXT, max_allowed INTEGER)
RETURNS INTEGER AS $$
DECLARE
...
-- ============================================
DROP FUNCTION IF EXISTS reactivate_properties_on_upgrade(TEXT, INTEGER);
DROP FUNCTION IF EXISTS reactivate_properties_on_upgrade(UUID, INTEGER);

CREATE OR REPLACE FUNCTION reactivate_properties_on_upgrade(user_uuid TEXT, new_max_properties INTEGER)
RETURNS INTEGER AS $$
DECLARE
...
-- ============================================
DROP FUNCTION IF EXISTS expire_all_expired_plans();
