-- Fix para las funciones RPC de planes
...
-- Recrear get_user_plan_limits con sintaxis correcta
...
BEGIN
  ...
  -- Retornar límites según el plan usando IF-ELSIF
...
-- Recrear can_user_activate_property con sintaxis correcta
...
BEGIN
  ...
  -- Determinar límite según el plan usando IF-ELSIF
  IF user_plan_tier = 'free' THEN
    max_props := 5;
  ELSIF user_plan_tier IN ('professional', 'pro') THEN
    max_props := 20;
...
-- Verificación: Ejecutar para ver si funciona
-- SELECT * FROM get_user_plan_limits('TU_USER_ID_AQUI');
