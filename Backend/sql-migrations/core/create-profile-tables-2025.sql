-- =====================================================
-- MIGRACIÓN: Tablas para Perfil de Usuario Real - 2025
-- =====================================================
...
CREATE TABLE IF NOT EXISTS profile_views (
    ...
    profile_user_id UUID NOT NULL REFERENCES "User"(id) ON DELETE CASCADE,
    viewer_user_id UUID REFERENCES "User"(id) ON DELETE SET NULL, -- NULL si es anónimo
...
CREATE TABLE IF NOT EXISTS user_ratings (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    rated_user_id UUID NOT NULL REFERENCES "User"(id) ON DELETE CASCADE,
    rater_user_id UUID NOT NULL REFERENCES "User"(id) ON DELETE CASCADE,
    property_id UUID REFERENCES properties(id) ON DELETE SET NULL,
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'profile_views') AND
       ...
       EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_activity_log') THEN
        RAISE NOTICE 'ÉXITO: Todas las tablas del perfil se crearon correctamente';
    ELSE
