-- ============================================================================
-- AUDITORÍA AVATARES - BASADA EN SCHEMA REAL
-- Fecha: 8 de Enero 2025
-- Schema Real Descubierto:
--   user_profiles (id, display_name, avatar_url, updated_at)
--   users (id, avatar, profile_image, logo_url, ...)
-- ============================================================================

-- ============================================================================
-- QUERY 1: AUDITORÍA COMPLETA DE AVATARES (SCHEMA REAL)
-- ============================================================================
...
SELECT 
  ...
  -- Fuente del avatar
  CASE 
    WHEN up.avatar_url IS NOT NULL THEN 'user_profiles.avatar_url ✅'
    WHEN u.avatar IS NOT NULL THEN 'users.avatar ⚠️'
    WHEN u.profile_image IS NOT NULL THEN 'users.profile_image ⚠️'
    WHEN u.logo_url IS NOT NULL THEN 'users.logo_url ⚠️'
    ELSE 'ninguna (usará ui-avatars.com)'
  END as fuente_avatar

FROM users u
LEFT JOIN user_profiles up ON up.id = u.id  -- ✅ CORREGIDO: up.id = u.id
...
FROM users u
LEFT JOIN user_profiles up ON up.id = u.id  -- ✅ CORREGIDO
WHERE up.id IS NULL
ORDER BY u.created_at DESC;
...
SELECT 
  ...
  COUNT(*)
FROM users u
INNER JOIN user_profiles up ON up.id = u.id  -- ✅ CORREGIDO
UNION ALL
SELECT 
...
-- ============================================================================
-- QUERY 6: USUARIOS ESPECÍFICOS (Carlos y norbi4050)
-- ============================================================================
...
SELECT 
  ...
  CASE 
    ...
    ELSE '✅ Avatar configurado en user_profiles'
  END as diagnostico
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (SELECT 1 FROM users WHERE id::text = split_part(so.name, '/', 1))
      THEN '✅ Usuario existe'
    ELSE '❌ Usuario NO existe (archivo huérfano)'
...
WHERE so.bucket_id = 'avatars'
  AND so.name ~ '^[a-f0-9-]{36}/avatar\.(jpg|jpeg|png|webp|gif)$'  -- Solo estructura correcta
ORDER BY so.created_at DESC;

-- ============================================================================
-- QUERY 8: ARCHIVOS DE PRUEBA A LIMPIAR
-- ============================================================================
...
SELECT 
  ...
  so.created_at,
  (so.metadata->>'size')::bigint as tamano_bytes,
  '⚠️ Archivo de prueba - Se puede eliminar' as nota
FROM storage.objects so
WHERE so.bucket_id = 'avatars'
  AND (so.name LIKE '%smoketest%' OR so.name LIKE '%test%' OR so.name LIKE '%demo%')
ORDER BY so.created_at DESC;
