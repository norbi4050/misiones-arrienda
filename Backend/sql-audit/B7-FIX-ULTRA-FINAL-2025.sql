-- ============================================================================
-- B7 - FIX ULTRA FINAL: Todos los errores corregidos
-- Fecha: Enero 2025
-- Errores corregidos:
-- 1. date_trunc no es IMMUTABLE
-- 2. operator text = uuid en política RLS
-- 3. COUNT(DISTINCT) en GROUP BY de kpi_plan_upgrades_daily
-- ============================================================================
...
-- ============================================================================
-- PASO 4: CREAR ÍNDICES (FIX: Remover date_trunc de índice)
-- ============================================================================
...
-- FIX: Removido idx_ae_day con date_trunc (no es IMMUTABLE)
...
-- ============================================================================
-- PASO 5: HABILITAR RLS (FIX: Cast explícito en política)
-- ============================================================================
...
-- FIX: Simplificar política admin - solo verificar si es service_role o bypass RLS
-- Los admins pueden usar service_role key para ver todos los eventos
CREATE POLICY "analytics_select_admin" 
ON public.analytics_events FOR SELECT
USING (
    -- Permitir si el usuario tiene email específico de admin
    -- O simplemente permitir a todos ver sus propios eventos (ya cubierto por analytics_select_own)
    -- Esta política es redundante, la removemos
    false
);

-- Nota: Los administradores deben usar service_role key para ver todos los eventos
-- service_role bypasses RLS automáticamente

-- ============================================================================
-- PASO 6: VISTAS KPI SPRINT E
-- ============================================================================
...
-- FIX: Remover COUNT(DISTINCT user_id) del SELECT y moverlo a subquery
CREATE VIEW public.kpi_plan_upgrades_daily AS
WITH daily_upgrades AS (
    SELECT 
        date_trunc('day', event_time)::date as day,
        user_id,
...
FROM daily_upgrades
GROUP BY day, from_plan, to_plan
...
BEGIN
    ...
    RAISE NOTICE '✅ Columnas: % (esperado: 14)', column_count;
    RAISE NOTICE '✅ Índices: % (esperado: 9)', index_count;
    RAISE NOTICE '✅ Políticas RLS: % (esperado: 3)', policy_count;
    RAISE NOTICE '✅ Vistas KPI: % (esperado: 15)', view_count;
    
    IF column_count = 14 AND index_count = 9 AND policy_count = 3 AND view_count = 15 THEN
