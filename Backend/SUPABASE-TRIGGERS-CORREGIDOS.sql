...
AS $function$
BEGIN
  -- NOTA: Supabase Auth ya crea automáticamente en 'profiles'
  -- Solo necesitamos crear en nuestra tabla personalizada 'User' si es necesario
  
  -- Crear usuario en la tabla User personalizada (opcional)
...
AS $function$
BEGIN
  -- NOTA: Supabase Auth actualiza automáticamente 'profiles'
  -- Solo actualizamos nuestra tabla personalizada 'User'
  
  -- Actualizar usuario en la tabla User personalizada
  UPDATE public."User" 
  SET 
...
AS $function$
BEGIN
  -- NOTA: Supabase Auth elimina automáticamente de 'profiles'
  -- Solo eliminamos de nuestra tabla personalizada 'User'
...
AS $function$
BEGIN
  -- Solo crear perfil de comunidad si se solicita explícitamente
...
-- Políticas para profiles (tabla de Supabase Auth)
...
BEGIN
  ...
  RAISE NOTICE '   - Supabase Auth crea automáticamente en tabla profiles';
  RAISE NOTICE '   - El trigger crea automáticamente en tabla User personalizada';
