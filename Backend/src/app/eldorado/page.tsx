...
export default async function EldoradoPage() {
  // Get properties for Eldorado
  const response = await getProperties({ city: 'Eldorado' })
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
              <div className="bg-white/10 rounded-lg p-4">
                <div className="text-2xl font-bold">{eldoradoProperties.length}</div>
                <div className="text-sm opacity-90">Propiedades Disponibles</div>
              </div>
              <div className="bg-white/10 rounded-lg p-4">
                <div className="text-2xl font-bold">
                  ${eldoradoProperties.length > 0 
                    ? Math.min(...eldoradoProperties.map((p: Property) => p.price)).toLocaleString()
                    : '0'
                  }
                </div>
              ...
              <div className="bg-white/10 rounded-lg p-4">
                <div className="text-2xl font-bold">
                  {new Set(eldoradoProperties.map((p: Property) => p.propertyType)).size}
                </div>
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          {eldoradoProperties.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
              {eldoradoProperties.map((property: Property) => (
                <PropertyCard 
                  key={property.id} 
                  id={property.id}
