...
export default function MisPublicacionesClient({ initialItems }: Props) {
  ...
  async function toggleFeatured(id: string, currentFeatured: boolean) {
    setBusyId(id)
    try {
      const resp = await fetch(`/api/properties/${id}/featured`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      toast.success(currentFeatured ? 'Propiedad desmarcada como destacada' : '⭐ ¡Propiedad destacada!')
    } catch (e: any) {
            ...
            >
              ...
              <div className="flex gap-2 w-full md:w-auto flex-wrap">
                ...
                {(p.status === 'PUBLISHED' || p.status === 'AVAILABLE') && (
                  <button
                    onClick={() => toggleFeatured(p.id, p.featured ?? false)}
                    ...
                    title={p.featured ? 'Quitar destacado' : 'Destacar propiedad'}
                  >
